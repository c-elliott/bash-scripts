#!/bin/bash
# Filename...: rhel7-ad-winbind.sh
# Description: Configures AD authentication on RHEL7 via Winbind (idmap_ad)
...
if [ $? -eq 0 ]; then
  ...
  yum -y install samba samba-common samba-winbind samba-winbind-clients krb5-workstation oddjob ntp
  ...
  cat <<< "
    # Generated by rhel7-ad-winbind.sh (`date`)
    # ------------------------------------------------------------------------
  ...
  # Test winbind
  if [ `systemctl status winbind | grep -c running` -eq 0 ]; then
